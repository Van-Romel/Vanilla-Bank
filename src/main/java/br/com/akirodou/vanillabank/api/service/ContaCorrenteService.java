...
public class ContaCorrenteService {
	...
	public ContaCorrenteEntity save(ContaCorrentPostDTO dto) {
		...
		String cartaoNumero = CONTA_CORRENTE_NUMBER_STARTS_WITH + String.format("%04d", random.nextInt(9999));
		while (contaCorrenteRepository.existsByCartaoDeCredito(cartaoNumero)) {
			cartaoNumero = CONTA_CORRENTE_NUMBER_STARTS_WITH + String.format("%04d", random.nextInt(9999));
		}
	...
	public ContaCorrenteEntity findByClienteCpf(String cpf) {
		return contaCorrenteRepository.findByTitularId(clienteService.findByCpf(cpf).getId())

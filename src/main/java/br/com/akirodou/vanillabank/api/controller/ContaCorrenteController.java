...
public class ContaCorrenteController {
	...
	public ResponseEntity<ContaCorrenteEntity> post(@RequestBody ContaCorrentPostDTO contaCorrentPostDTO) {
		return ResponseEntity.status(HttpStatus.CREATED).body(contaCorrenteService.save(contaCorrentPostDTO));
	}
...
//                .map(resp -> ResponseEntity.ok(resp)).orElse(ResponseEntity.notFound().build());
	...
	public ResponseEntity<?> depositar(@PathVariable Long id, @RequestBody ValorDTO dto) {
		return ResponseEntity.status(HttpStatus.OK).body(contaCorrenteService.depositar(id, dto));
	}

	@PutMapping("/sacar/{id}")
	public ResponseEntity<?> sacar(@PathVariable Long id, @RequestBody ValorDTO dto) {
		return ResponseEntity.status(HttpStatus.OK).body(contaCorrenteService.sacar(id, dto));
	}
